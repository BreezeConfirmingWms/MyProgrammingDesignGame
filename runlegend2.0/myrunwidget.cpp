...
using namespace std;
bool gjug=0,magjug=0,mokjug=0,emojug=0,keyjug=1;
int magnum=1;
int keynum=0;
             ...
             break;
                 ...
                 if((*i)->jugcollision(r->getx()-5,r->gety()-5,r->getwid()-5,r->getwid()-5)){
                    ...
                    if(shradjug){
                     ...
                     r->hptimer.stop();
                     sdlg->exec();
                     QString inmrtn=QString::number(rscore);
                     qDebug()<<inmrtn;
                     r->setscore(rscore);
                     if(rtn){
        ...
        for(auto i=bar10.begin();i!=bar10.end();i++){
            if((*i)->done()){
                ...
                if((*i)->jugcollision(r->getx()-5,r->gety()-5,r->gethei()-5,r->gethei()-5)){
                    if(magjug){         cntmp=0;
                                        ...
                                        QMessageBox box;
                                        box.about(this,"圣树掉落的魔法书",montxt);
            ...
            for(auto j=barmon.begin();j!=barmon.end();j++){
                if((*j)->done()){
                    ...
                    if((*j)->jugcollision(r->getx()-5,r->gety()-5,r->getwid()-5,r->getwid()-5)){
                        srand(time(NULL));
                        int rcore=250+rand()%180;
                        r->setscore(rcore);
                        r->increasehp(rscore*2-rand()%100);
                        barmon.clear();
                        break;
            ...
            for(auto j=bardemo.begin();j!=bardemo.end();j++){
                ...
                if((*j)->done()){
                    ...
                    if((*j)->jugcollision(r->getx()-10,r->gety()-10,r->getwid()-10,r->getwid()-10)){
                        r->reducehp();
                        r->reducehp();
                    }
                    if(!bardemo.empty()){
...
void gamewidget::addBarriers()
{
    ...
    if(Lastbequest_time>=900){
        int x=this->width()+10;
        int y=ground_Y-95-rand()%250;
